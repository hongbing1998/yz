...
(function ($) {
    $.uedWidget('ued.Grid', {
        ...
        _create: function () {
            ...
            $el.addClass("ui-table");
            //=======判断单元格内容是否允许换行========
            if (options.nowrap) {
                $el.addClass("ui-table-nowrap");
            }
        ...
        _createTableDom: function ($el, options, _define) {
            ...
            //********绘制操作条*******
            var $optBar = $("<div class='ui-table-optBar'></div>");
            ...
            if (settings) {
                var $setting = $("<div class='ui-table-setting'></div>");
            ...
            //******绘制表格panel区域********
            var $tablePanel = $domCache.$tablePanel = $('<div class="ui-table-panel __panel"></div>');
            //绘制表格头部
            var $tableThead = $domCache.$tableThead = $('<div class="ui-table-thead"><div class="ui-table-thead-shadow"></div></div>');
            //是否有checkbox属性
            if (options.checkbox) {
                ...
                if (!options.checkbox.single) {
                    $table.find('.ui-table-thTitle').append('<a href="javascript:;" class="ue-state-default"><span class="__optPanelBtn ui-icon ui-icon-triangle-1-s"></span></a>');
            ...
            //绘制表格头部显示列table
            var $headMain = $domCache.$headMain = $('<div class="__theadMain ui-table-main"><table><thead><tr></tr></thead></table></div>');
            $tableThead.append($headMain);
            //绘制表格内部右侧操作区域
            if (options.operator) {
                $el.addClass('ui-table-fixed-opt');
                if (options.headCount == 1) {
                    var $theadFianl = $('<div class="__theadFinal table-finale-fixed"><table><thead><tr><th class="ui-table-operation"><div class="ui-table-thTitle">操作</div></th></tr></thead></table></div>');
                } else {
                    var $theadFianl = $('<div class="__theadFinal table-finale-fixed"><table><thead><tr><th class="ui-table-operation" rowspan="2"><div class="ui-table-thTitle">操作</div></th></tr></thead></table></div>');
            ...
            var $tableTbody = $domCache.$tableTbody = $('<div class="ui-table-tbody ui-scroll-webkit" style="height:' + height + '"></div>');
            if (options.checkbox) {
            ...
            var $tableMain = $('<div class="__tbodyMain ui-table-main"><table><thead></thead><tbody class="__data"></tbody></table></div>');
            $tableTbody.append($tableMain);

            //绘制无数据和加载中
            var $tableNoData = $domCache.$tableNoData = $('<div style="display:none;" class="ui-nodata-tip">无数据</div>');
            $tableTbody.append($tableNoData);
            var $tableLoading = $domCache.$tableLoading = $('<div style="display:none;" class="ui-table-loading">数据加载中...</div>');
            $tableTbody.append($tableLoading);

            $tablePanel.append($tableTbody);
            //绘制表格横滚动条区域
            var $tableScroll = $('<div class="__tablescroll ui-table-scroll-horizontal ui-scroll-webkit"><div></div></div>')
            $tablePanel.append($tableScroll);
            var $division = $domCache.$division = $('<div class="ui-table-division"></div>');
        ...
        _setSettingData: function ($setting, index, settings) {
            ...
            if (_define.customIndex >= 0) {//有表格定制
                ...
                var $customBuiltTable = $('<div class="custom-built-table-dialog"><div class="custom-built-table">' +
                    '<div class="custom-built-table-tip ue-text-level3"><span class="ui-icon2 ui-icon2-tip"></span><label>小贴士：</label>直接点选您想定制的列，再次点击表示取消选择</div>' +
                    '<h6>' +
                    '选择我要定制的列' +
        ...
        setData: function (data_, head_) {
            ...
            $el.find('.ui-table-thead').find('input[type=checkbox]').attr('checked', false);
        },
        ...
        _setCustomPanelData: function (tableHead, sequence) {
            var tempLi = [], $customTable = this.$domCache.$customTable, self = this, options = this.options,
                $el = this.element;
            ...
            $customTable.find('.ui-dialog-selected').text(sequence.length);
            $customTable.find('.built-table-col').find("ul").empty().append(tempLi.join(''));
            this._initSequence();
            //绑定恢复默认事件
            $customTable.find('.defaultSequence').click(function () {
                var _define = self._define, $selected = $customTable.find('.ui-dialog-selected');
            ...
            $customTable.find(".built-table-col").find("li").click(function (e) {
                var sequence = self._define.sequence, $selected = $customTable.find('.ui-dialog-selected');
                    ...
                    buttons: [{
                        ...
                        callBack: function ($dialog, $mask) {
                            if (self._define.currentSequence.toString() != self._define.tempSequence.toString()) {//增加判断定制信息是否改变
                                var currentPage = $(".ui-pagination", $el).uedPagination('option', 'currentPage');
        ...
        _initSequence: function (sequence) {
            var $el = this.element, options = this.options, self = this, tableHead = this._define.tableHead,
                sequence = sequence || this._define.currentSequence, $customTable = this.$domCache.$customTable;
            ...
            function downHandler(mde) {
                mde.preventDefault();
                if ($(mde.target).hasClass('ue-state-default') || $(mde.target).hasClass('ui-icon-close')) {
                    return false;
                }
                var $dragging = $(this);
            ...
            $builtTableSelected.on('click.uedGrid', 'span', function () {//添加排序界面删除选中的项事件。
                var $li = $(this).parents('li'), sequence = self._define.tempSequence,
                    $customBuiltCol = $customTable.find('.built-table-col');
                var index = $li.index();
                if ($builtTableSelected.find('li').size() > 1) {
                    $li.remove();
        ...
        _setTableHeadData: function ($el, _define) {
            ...
            if (options.headCount == 1) {
                var $tableHeadTr = $el.find('.__theadMain tr').empty(),
                    $tableMainHead = $el.find('.__tbodyMain thead').empty().append("<tr></tr>").find('tr'), thArr = [];
                for (var i = 0, len = tableHead.length; i < len; i++) {
                    var $th = $('<th><div class="ui-table-th" style="width:' + (tableHead[i].width - 30) + 'px"><span class="ui-table-thTitle">' + tableHead[i].label + '</span></div></th>');

                    if (tableHead[i].sortable == "default") {
                        $('<a href="javascript:;" class="ui-icon2 ui-icon2-sort"></a>').insertAfter($th.find('.ui-table-thTitle'));
                    } else if (tableHead[i].sortable == "desc") {
                        $('<a href="javascript:;" class="ui-icon2 ui-icon2-sort-desc"></a>').insertAfter($th.find('.ui-table-thTitle'));
                    } else if (tableHead[i].sortable == "asc") {
                        $('<a href="javascript:;" class="ui-icon2 ui-icon2-sort-asc"></a>').insertAfter($th.find('.ui-table-thTitle'));
                    }
                    if (options.widthResize) {
                        $th.children().append('<span class="ui-table-drag"></span>');
                    }
                    if (tableHead[i].name) {
                ...
                $tableHeadTr.append('<th class="ui-table-blank">&nbsp;</th>');
                $tableMainHead.append('<th class="ui-table-blank">&nbsp;</th>');//插入到表格体中的head
            } else if (options.headCount == 2) {
                var $tableHead = $el.find('.__theadMain thead'),
                    $tableMainHead = $el.find('.__tbodyMain thead').empty();
                $el.find('.__theadMain tr').empty();
                $el.addClass("ui-table-doubleTh");
                $tableHead.append('<tr class="ui-table-secondHd"></tr>');
                var indexCount = 0, $thObj1 = $tableHead.find('tr').eq(0), $thObj2 = $tableHead.find('tr').eq(1);
                for (var i = 0; i < tableHead.length; i++) {
                    if (tableHead[i].children) {
                        $thObj1.append('<th colspan="' + tableHead[i].children.length + '"><div class="ui-table-th"><span class="ui-table-thTitle">' + tableHead[i].label + '</span><a href="javascript:;"></a></div></th>');
                        for (var j = 0; j < tableHead[i].children.length; j++) {
                            var childrenData = tableHead[i].children[j];
                            $thObj2.append('<th indexCount="' + indexCount++ + '"><div class="ui-table-th"><span class="ui-table-thTitle">' + childrenData.label + '</span><a href="javascript:;"></a></div></th>');
                        ...
                        $thObj1.append('<th rowspan="2" indexCount="' + indexCount++ + '"><div class="ui-table-th"><span class="ui-table-thTitle">' + tableHead[i].label + '</span><a href="javascript:;"></a></div></th>');
                ...
                $thObj1.append('<th rowspan="2" class="ui-table-blank">&nbsp;</th>');
            ...
            for (var i = 0, rowLen = tableBody.length; i < rowLen; i++) {//行遍历
                ...
                for (var j = 0, fieldLen = tableHead.length; j < fieldLen; j++) {
                    ...
                    if (tableHead[j].minWidth) {
                        if (tableHeadJ.name) {
                            tdArr.push('<td minWidth="minWidth" name="' + tableHeadJ.name + '"><div class="ui-table-td" style="width:' + (tableHeadJ.width - 16) + 'px">' + rowData[j] + '</div></td>');
                        } else {
                            tdArr.push('<td minWidth="minWidth"><div class="ui-table-td" style="width:' + (tableHeadJ.width - 17) + 'px">' + rowData[j] + '</div></td>');
                        }

                    } else {
                        if (tableHeadJ.name) {
                            tdArr.push('<td name="' + tableHeadJ.name + '"><div class="ui-table-td" style="width:' + (tableHead[j].width - 17) + 'px">' + rowData[j] + '</div></td>');
                        } else {
                            tdArr.push('<td><div class="ui-table-td" style="width:' + (tableHead[j].width - 17) + 'px">' + rowData[j] + '</div></td>');
                        }

                    }

                }
                tdArr.push('<td class="ui-table-blank"><div class="ui-table-td"></div></td>');
        ...
        _createTableBodyFinal: function ($el, options, tableBody, isNoEmpty) {
            var $tbodyFinal = this.$domCache.$tbodyFinal, isPanel = options.operator.type == 'panel',
                _define = this._define;
            if (!isNoEmpty) {
            ...
            for (var i = 0, len = tableBody.length; i < len; i++) {
                ...
                if (isPanel) {//操作板情况
                    if (this._isType(optArray, 'Array')) {
                        trArray.push('<tr attr="38"><td class="ui-table-operation"><a href="javascript:;" class="ue-state-default __operatorPanelBtn" operindex="' + i + '"><span class="ui-icon ui-icon-circle-triangle-s"></span></a></td></tr>');

                    } else {
                        trArray.push('<tr attr="38"><td class="ui-table-operation"><span></span></td></tr>');
                    }
                } else {//非操作板情况
                    if (this._isType(optArray, 'Array')) {
                        var optList = [];
        ...
        _bindCheckboxEvent: function ($el, $tbodyFront) {
            var checkedLen = 0, $theadInput = $el.find('.ui-table-thead').find('input[type=checkbox]');
        ...
        _bindEvent: function ($el, options) {
            ...
            //表格操作条事件
            $el.find(".ui-table-optBar").on("click.uedGrid", 'button', function () {
                if (!$(this).hasClass("ue-state-disable")) {
                    if ($(this).parent().hasClass("ui-table-optBar")) {
            ...
            $domCache.$tableScroll.bind("scroll.uedGrid", function () {
                $el.find(".ui-table-main table").css("margin-left", 0 - $(this).scrollLeft());
            });

            $el.find('.ui-table-tbody').bind('scroll.uedGrid', function () {
        ...
        _bindTableEvent: function () {
            ...
            //点击排序返回index值
            $el.find(".__theadMain").find(".ui-icon2").each(function (i) {
                $(this).unbind("click");
                $(this).bind("click", function () {
                    if (self.isLoadding) {
                        return false;
                    }
                    $el.find(".__theadMain").find(".ui-icon2").not($(this)).removeAttr("class").addClass("ui-icon2 ui-icon2-sort");
                    var tmpname = $(this).parent().parent("th").attr("name");
                    if (!options.defaultSortType || options.defaultSortType == 'desc') {

                        if ($(this).hasClass("ui-icon2-sort-desc")) {
                            $(this).removeClass("ui-icon2-sort-desc").addClass("ui-icon2-sort-asc");
                            self._trigger("sortCallBack", event, tmpname, (i + 1), "asc", $el);
                        } else {
                            $(this).removeClass("ui-icon2-sort-asc").addClass("ui-icon2-sort-desc");
                            self._trigger("sortCallBack", event, tmpname, (i + 1), "desc", $el);
                        }
                    } else {

                        if ($(this).hasClass("ui-icon2-sort-asc")) {
                            $(this).removeClass("ui-icon2-sort-asc").addClass("ui-icon2-sort-desc");
                            self._trigger("sortCallBack", event, tmpname, (i + 1), "desc", $el);
                        } else {
                            $(this).removeClass("ui-icon2-sort-desc").addClass("ui-icon2-sort-asc");
        ...
        _adjustTable: function ($el, options) {
            ...
            $el.find(".ui-table-main table").css("margin-left", 0);
        ...
        _adjustWidth: function ($el) {
            var options = this.options, _define = this._define, $blankTh = $el.find('th.ui-table-blank'),
                $blankTd = $el.find('td.ui-table-blank'), $domCache = this.$domCache,
                $tableTbody = $domCache.$tableTbody, $tbodyFront = $domCache.$tbodyFront,
                $tbodyFinal = $domCache.$tbodyFinal;

            _define.currentTableWidth = _define.tableWidth + _define.blankWidth + 6;//记录表格的实际宽度即为当前宽度
            var $tableMain = $el.find('.__tbodyMain');
            var tableMainWidth = $el.find('.ui-table-tbody').width();
            //判断是否有垂直滚动条
            if ($tableTbody.height() < $tableTbody.children().height() && options.height != 'auto') {
                $el.addClass('ui-table-hasScroll');
            } else {
                $el.removeClass('ui-table-hasScroll');
            }

            if (!-[1,]) {//IE宽度处理流程

                if ($el.hasClass('ui-table-hasScroll')) {
                    tableMainWidth -= 17;
                }
            }
            if (options.checkbox) {
            ...
            if (this.$minWidthTh) {//如果有minWidth
                var $uiTableMinTh = $el.find('.minWidth').find('.ui-table-th'), reaWidth = $uiTableMinTh.width();
                if (tempWidth > 0) {//无水平滚动条,将tempWidth的值给minWidth

                    $uiTableMinTh.width(tempWidth + this.$minWidthTh.width);
                    //更新当前表格实际宽度
                    ...
                    $domCache.$division.removeClass('ui-table-hScroll');
                } else {
                    $uiTableMinTh.width(this.$minWidthTh.width);
                    var tempMinWidth = $el.find('.__theadMain').find('th.minWidth').children().width();
                    $el.find('td[minWidth]').each(function () {
                        $(this).children().width(tempMinWidth + 14);
                    });
                    $domCache.$tableScroll.show().children().width(_define.currentTableWidth);
                    $domCache.$division.addClass('ui-table-hScroll');
                }
                ...
                if (tempWidth > 0) { //无水平滚动条，直接将tempWidth的值给空白列
                    ...
                    $el.find('.ui-table-blank').attr('width', blankWidthTemp);
        ...
        _adjustOperator: function ($el, options) {
            ...
            if (options.operator) {//只有当操作为normal生效
                ...
                if (options.operator.type == "normal") {
                    //无滚动条
                    $el.find(".__tbodyFinal,.__tbodyFinal table,.__theadFinal,.__theadFinal table").width(options.operator.width - 1).find(".ui-table-operation").width(options.operator.width - 6);
                    $el.find(".ui-table-main").css("right", options.operator.width);
                    $tableScroll.css("margin-right", options.operator.width);
                    if ($el.hasClass("ui-table-hasScroll") && options.height != 'auto') {
                    ...
                    if (!-[1,] && $el.hasClass("ui-table-hasScroll")) {
        ...
        _controlShowOrHide: function ($el, options) {
            ...
            if (!options.tableBody) {//if tableBody为null
                if (!options.tableHead || !options.tableHead.length) { //无表格头时
                    ...
                    $el.children(".ui-table-optBar").find("button").addClass("ue-state-disable");
                    $el.children(".ui-table-optBar").find("button[alwaysEnable]").removeClass("ue-state-disable");
                    $domCache.$tableScroll.hide();
                    $domCache.$tableLoading.hide();
                    $el.find(".ui-pagination").hide();
                    $el.children(".__panel").children("div:not(.ui-table-division)").hide();
                ...
                $domCache.$tableTbody.show().children(".ui-nodata-tip").hide();
                $el.children(".__panel").children(".ui-table-thead").show();
                $el.children(".ui-table-optBar").find("button").removeClass("ue-state-disable");
                $domCache.$tableScroll.show();
                $domCache.$tableLoading.hide();
                $el.find(".ui-pagination").show('visibility');
                $el.find(".ui-pagination").show();
        ...
        _tableNoData: function ($el, options, $domCache) {
            ...
            $el.children(".ui-table-optBar").find("button:not([alwaysEnable])").addClass("ue-state-disable");
            $el.children(".ui-table-optBar").find("button[alwaysEnable]").removeClass("ue-state-disable");
            $el.children(".__panel").children("div:not(.ui-table-division)").show();
            ...
            if (!options.tableHead || !options.tableHead.length) {
                $el.find(".ui-table-thead").hide();
                $domCache.$tableNoData.hide()
            } else {
                $el.find(".ui-table-thead").show();
            }
            $el.children(".__panel").find('.__tbodyMain').hide();
            $domCache.$division.hide();
            $el.find('.ui-table-thead').children().show();
        },
        /*
         * 调整表格高度
         */
        ...
        _dragTable: function ($el) {
            ...
            var endX;
            //鼠标按下事件
            $el.find('.ui-table-drag').bind('mousedown', function (e) {
                e.preventDefault();
                $dragging = $(this);
                //记录拖拽前的坐标值
                ...
                function dragMove(e) {
                    e.preventDefault();
                    //改变拖动的位置
                    $dragging.css('right', startX - e.clientX - 1).addClass('ui-table-dragging');
                }
                ...
                function dragUp(e) {
                    ...
                    $dragging.removeClass('ui-table-dragging');
                    ...
                    //存储的宽度改变前td的宽度
                    var sourceWidth = $el.find(".__theadMain").children("table").children("thead").children("tr").children("th").eq(index).children(".ui-table-th").width();
                    //存储的拖拽后的值
                    var targetWidth = sourceWidth + distance;
                    var minWidth = $el.find(".__theadMain").children("table").children("thead").children("tr").children("th").eq(index).children(".ui-table-th").children(".ui-table-thTitle").width();
                    ...
                    $el.find('thead').each(function (i) {
                        var $thDiv = $(this).find('th').eq(index).find('.ui-table-th');
                        changeWidth = $thDiv.width();
                        $thDiv.width(targetWidth);
                        changeWidth = targetWidth - changeWidth;
                    });
                    //调整空白列的宽度
                    var blankWidth = $el.find('.__theadMain th.ui-table-blank').attr('width');
                    if (blankWidth == undefined) {
                        blankWidth = $el.find('.__theadMain th.ui-table-blank').outerWidth();
                    }
                    ...
                    $el.find('.ui-table-blank').width(tempBlankWidth);
                    $el.find('.ui-table-blank').attr('width', tempBlankWidth);//保存width，防止获取不准确
                    //设置拖拽条的坐标
                    ...
                    if (!self.nowrap && (self.options.checkbox || self.options.operator)) {
                        ...
                        //self._adjustWidth($el);
                        if (!-[1,]) {
                            self._adjustOperator($el, self.options);
                        }
                    } else if (!self.nowrap) {
                        self._adjustHeight($el, self.options);
                    }
        ...
        setOperatorData: function (operatorData, index, name) {
            var options = this.options, $el = this.element, self = this, _define = this._define,
                isPanel = options.operator.type == 'panel';
            if (index) {
                //改变与index对应的行的操作板
                if (name) {
        ...
        _changeOperator: function (isPanel, newOperatorData, rowIndex, operatorIndex) {
                    ...
                    $el.find('.__tbodyFinal .__bodyfinal').find('tr').each(function (i, elemt) {
                        if (!newOperatorData[i] || newOperatorData[i].length == 0) {
                            ...
                            if (!$(this).find('a').size() && isPanel) {
                                $(this).find('td').append('<a href="javascript:;" class="ue-state-default __operatorPanelBtn" operindex="' + i + '"><span class="ui-icon ui-icon-circle-triangle-s"></span></a>').find('span:not(.ui-icon)').remove();
                                ...
                                $(this).find('td').find('a').remove().end().append(thA).find('span:not(.ui-icon)').remove();
                            }
                        }
                    });
                }
            ...
            var $operTr = $el.find('.__tbodyFinal .__bodyfinal').find('tr').eq(rowIndex),
                newOperatorData = newOperatorData[rowIndex];
            //是操作板的情况
            if (isPanel) {
                if (!newOperatorData || newOperatorData.length == 0) {
                    $operTr.find('a').hide();
                    ...
                    if (!$operTr.find('a').size()) { //如果此行初始化就没有生成此节点，则这里插入该节点。
                        $operTr.find('td').append('<a href="javascript:;" class="ue-state-default __operatorPanelBtn" operindex="' + rowIndex + '"><span class="ui-icon ui-icon-circle-triangle-s"></span></a>').find('span:not(.ui-icon)').remove();
                        //重新设置是触发操作板对象。
                        $operatorPanel.uedOptPanel('setOpter', [$el.find(".__operatorPanelBtn")]);
                    }
                    $operTr.find('a').show();
                ...
                if (!newOperatorData || newOperatorData.length == 0) {
                    ...
                    $operTr.find('td').find('a').remove().end().append(thA).find('span:not(.ui-icon)').remove();
        ...
        _uedResize: function (resizeHandle) {
            ...
            $(window).on('resize.uedGrid', function (e) {
                ...
                var isHide = !$el.is(':visible'),
                    isOutWindow = $el.hasClass('ui-outside-hidden') || !!$el.parents('.ui-outside-hidden').size();
        ...
        resize: function () {
            var $el = this.element, isHide = !$el.is(':visible'),
                isOutWindow = $el.hasClass('ui-outside-hidden') || !!$el.parents('.ui-outside-hidden').size();
            if (isHide || isOutWindow) {
                return;
            }
