BUI.use(["bui/menu", "bui/tab"], function (h, l) {
    ...
    var p = "dl-selected", c = "ks-hidden", y = "dl-last", x = "dl-hover", n = "nav-item", s = "dl-second-slib",
        f = "dl-tab-item", t = "dl-collapse", v = "dl-hide-current", u = "data-index", j = 145;
    ...
    function i(B, E, H, F, A) {
        var I = this, C = new h.SideMenu(H), D = new l.NavTab(E), J = $(H.render), z = J.next("." + s + "-con"),
            G = J.parents("." + f);
        if (z) {
            z.on("click", function () {
                G.toggleClass(t)
    ...
    function d() {
        var D = location.hash, C = 0, z = "", A = D.indexOf("/"), B = null;
        if (!D) {
            return null
        }
        if (A >= 0) {
        ...
        return {moduleId: C, pageId: z, search: B}
    }
    ...
    w.ATTRS = {
        ...
        urlSuffix: {value: ".html"}
    };
    BUI.extend(w, BUI.Base);
    BUI.augment(w, {
        openPage: function (O) {
            var J = this, E = O.moduleId || J._getCurrentModuleId(), C = O.id,
                L = O.title || "\u65b0\u7684\u6807\u7b7e\u9875", B = O.href, N = O.isClose, D = O.closeable,
                A = O.reload, P = O.search;
            var G = J._getModule(E);
            if (G) {
                var H = G.tab, F = G.menu, z = F.getItem(C), K = H.getActivedItem(), M = K ? K.get("id") : null,
                ...
                if (z) {
                    ...
                    H.addTab({id: C, title: L, href: B, sourceId: M, closeable: D}, A)
                }
                if (N) {
                    K.close()
                }
            ...
            A = A || [];
            var z = this, B = z._getModule(E);
            if (B) {
                var C = B.tab, D = G ? C.getItemById(G) : C.getActivedItem();
                if (D && D[F]) {
                    D[F].apply(D, A)
                }
            }
        }, _createModule: function (E) {
            var z = this, C = z._getModuleConfig(E), B = z.get("modules");
            if (!C) {
                return null
            }
            var E = C.id, A = "#J_" + E + "Tab", D = "#J_" + E + "Tree";
            module = new i(E, {render: A, height: a() - 5}, {
                render: D,
                items: C.menu,
                height: a() - 5
            ...
            var I = this, C = I.get("navItems"), B = I.get("hideItmes");
            if (C.length === 0) {
                return
            }
            $('<div class="nav-item-mask"></div>').appendTo($(C));
            var G = C.length, K = BUI.viewportWidth(), H = j, F = H * G, z = 0;
            if (F <= K) {
                return
            }
            $.each(C, function (L, M) {
            ...
            var B = this, A = B.get("hideList"), z = B.get("hideItmes");
            if (A) {
                return
            }
            var D = '<ul class="dl-hide-list ks-hidden"></ul>', C = $(D).appendTo("body");
            A = C;
            $.each(z, function (E, F) {
                $(F).appendTo(A)
            });
            ...
            var z = this, B = z.get("modulesConfig"), A = z.get("navContent");
            A.children().remove();
            $.each(B, function (D, E) {
                var F = E.id,
            ...
            var A = this, z = A.get("hideList"), B = A.get("navList");
            B.on("mouseover", function (E) {
                var D = o(E.target), C = $(D), F = null;
                if (C && C.hasClass(y) && z) {
                    F = C.offset();
                    F.top += 37;
                    F.left += 2;
            ...
            var z = this, D = d();
            if (D) {
                var B = D.pageId, E = D.search, C = z._getModuleIndex(D.moduleId);
                z._setModuleSelected(C);
                z._setPageSelected(C, B, true, E)
            } else {
                var A = z.get("currentModelIndex"), F = z._getModuleId(A);
                if (A == null) {
            ...
            var z = this, D = z._getCurrentModuleId(), B = z._getModule(D), A = "";
            if (B) {
            ...
            var z = this, A = z.get("lastShowItem");
            if (A === D) {
                return
            }
            var B = null, C = $(A);
            ...
            var H = this, D = H.get("navItems"), B = H.get("navTabs"), G = H.get("currentModelIndex");
            if (G !== F) {
                var z = H._getModuleId(F), A = null, C = H.get("lastShowItem"), I = true;
            ...
            var H = this, B = H._getModuleId(E) || E, C = H._getModule(B);
            if (C && F) {
                C.menu.setSelectedByField(F);
                var J = C.menu.getSelected(), D = C.tab, A = "", z = -1;
                if (J && J.get("id") === F) {
                    A = J.get("href");
                    A = K ? (b(A, K)) : A;
                    C.tab.addTab({id: J.get("id"), title: J.get("text"), closeable: J.get("closeable"), href: A}, !!G)
                } else {
                    if (F) {
                        var I = F.replace("-", "/");
                        if (I.indexOf("/") === -1) {
            ...
            var B = this, A = B.get("hideList"), z = B.get("hideItmes"), E = null, C = null, G = null, F = null;
